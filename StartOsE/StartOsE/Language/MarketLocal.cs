...
namespace StartOsE.Language
{
    public class MarketLocal
    {
        ...
        public string Label87 => OsLocalization.ConvertToLocString(
         "Eng:If true - then StartOsE stores in the OsData folder all the deal feed it receives. It's very resource intensive! Recommendation - false._" +
         "Ru:Если true - то StartOsE хранит в папке OsData всю ленту сделок которую получает. Это очень ресурсоёмко! Рекомендация - false._");
        ...
        public string Label89 => OsLocalization.ConvertToLocString(
         "Eng:If true - then StartOsE stores in the OsData folder the candlesticks it receives and generates in the process of work. Recommendation - true._" +
         "Ru:Если true - то StartOsE хранит в папке OsData свечи которые получает и формирует в процессе работы. Рекомендация - true._");
        ...
        public string Message76 => OsLocalization.ConvertToLocString(
            "Eng:Ninja not responding. Probably you have not active script StartOsEConnect in Ninja_" +
            "Ru:Ninja не отвечает. Вероятно у Вас не активен скрипт StartOsEConnect в Ninja _");
