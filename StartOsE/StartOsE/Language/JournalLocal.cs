/*
 * Your rights to use code governed by this license http://o-s-a.net/doc/license_simple_engine.pdf
 * Ваши права на использование кода регулируются данной лицензией http://o-s-a.net/doc/license_simple_engine.pdf
*/

namespace StartOsE.Language
{
    public class JournalLocal
    {
        public string PositionToolStripMenuItem1 => OsLocalization.ConvertToLocString(
            "Eng:Close all. Market_" +
            "Ru:Закрыть все по маркету_");

        public string PositionToolStripMenuItem2 => OsLocalization.ConvertToLocString(
            "Eng:New position_" +
            "Ru:Открыть новую_");

        public string PositionToolStripMenuItem3 => OsLocalization.ConvertToLocString(
            "Eng:Close selected_" +
            "Ru:Закрыть выбранную_");

        public string PositionToolStripMenuItem4 => OsLocalization.ConvertToLocString(
            "Eng:Modify selected_" +
            "Ru:Модифицировать выбранную_");

        public string PositionToolStripMenuItem5 => OsLocalization.ConvertToLocString(
            "Eng:Swap stop order_" +
            "Ru:Переставить стоп_");

        public string PositionToolStripMenuItem6 => OsLocalization.ConvertToLocString(
            "Eng:Swap profit order_" +
            "Ru:Переставить профит_");

        public string PositionToolStripMenuItem7 => OsLocalization.ConvertToLocString(
            "Eng:Delete position_" +
            "Ru:Удалить позицию_");

        public string PositionToolStripMenuItem8 => OsLocalization.ConvertToLocString(
            "Eng:Detail_" +
            "Ru:Детализация_");

        public string PositionToolStripMenuItem9 => OsLocalization.ConvertToLocString(
            "Eng:Delete selected_" +
            "Ru:Удалить выбранную_");

        public string PositionToolStripMenuItem10 => OsLocalization.ConvertToLocString(
            "Eng:Delete all_" +
            "Ru:Удалить все_");

        public string PositionToolStripMenuItem11 => OsLocalization.ConvertToLocString(
            "Eng:Save to file_" +
            "Ru:Сохранить в файл_");

        public string PositionToolStripMenuItem12 => OsLocalization.ConvertToLocString(
            "Eng:Find on chart_" +
            "Ru:Найти на графике_");

        public string PositionToolStripMenuItem13 => OsLocalization.ConvertToLocString(
            "Eng:Create position_" +
            "Ru:Создать позицию_");

        public string TitleJournalUi => OsLocalization.ConvertToLocString(
            "Eng:Journal_" +
            "Ru:Журнал_");

        public string Label1 => OsLocalization.ConvertToLocString(
            "Eng:Total profit_" +
            "Ru:Общая прибыль_");

        public string Label2 => OsLocalization.ConvertToLocString(
            "Eng:Long_" +
            "Ru:Лонг_");

        public string Label3 => OsLocalization.ConvertToLocString(
            "Eng:Short_" +
            "Ru:Шорт_");

        public string Label4 => OsLocalization.ConvertToLocString(
            "Eng:Volume Assemblage_" +
            "Ru:Ассемблирование объёма_");

        public string Label5 => OsLocalization.ConvertToLocString(
            "Eng:From_" +
            "Ru:От_");

        public string Label6 => OsLocalization.ConvertToLocString(
            "Eng:To_" +
            "Ru:До_");

        public string Label7 => OsLocalization.ConvertToLocString(
            "Eng:Reload_" +
             "Ru:Перезагрузить_");

        public string Label8 => OsLocalization.ConvertToLocString(
            "Eng:Chart Type_" +
            "Ru:Тип линии_");

        public string Label9 => OsLocalization.ConvertToLocString(
            "Eng:Group_" +
            "Ru:Группа_");

        public string Label10 => OsLocalization.ConvertToLocString(
            "Eng:Name_" +
            "Ru:Имя_");

        public string Label11 => OsLocalization.ConvertToLocString(
            "Eng:Class_" +
            "Ru:Класс_");

        public string Label12 => OsLocalization.ConvertToLocString(
            "Eng:On/Off_" +
            "Ru:Вкл/выкл_");

        public string Label13 => OsLocalization.ConvertToLocString(
           "Eng:New Bots Group Creation_" +
           "Ru:Создание новой группы_");

        public string Label14 => OsLocalization.ConvertToLocString(
          "Eng:Accept_" +
          "Ru:Принять_");

        public string Label15 => OsLocalization.ConvertToLocString(
          "Eng:Auto update_" +
          "Ru:Обновление_");

        public string Label16 => OsLocalization.ConvertToLocString(
          "Eng:Show_" +
          "Ru:Показывать_");

        public string TabItem1 => OsLocalization.ConvertToLocString(
            "Eng:Equity_" +
            "Ru:Эквити_");

        public string TabItem2 => OsLocalization.ConvertToLocString(
            "Eng:Statistics_" +
            "Ru:Статистика_");

        public string TabItem3 => OsLocalization.ConvertToLocString(
            "Eng:Drawdown_" +
            "Ru:Просадка_");

        public string TabItem4 => OsLocalization.ConvertToLocString(
            "Eng:Volume_" +
            "Ru:Объём_");

        public string TabItem5 => OsLocalization.ConvertToLocString(
            "Eng:Open positions_" +
            "Ru:Открытые позиции_");

        public string TabItem6 => OsLocalization.ConvertToLocString(
            "Eng:Closed positions_" +
            "Ru:Закрытые позиции_");

        public string GridColumn1 => OsLocalization.ConvertToLocString(
            "Eng:All_" +
            "Ru:Все_");

        public string GridColumn2 => OsLocalization.ConvertToLocString(
            "Eng:Long_" +
            "Ru:Лонг_");

        public string GridColumn3 => OsLocalization.ConvertToLocString(
            "Eng:Short_" +
            "Ru:Шорт_");

        public string GridRow1 => OsLocalization.ConvertToLocString(
            "Eng:Profit\\Loss_" +
            "Ru:Чистый П\\У_");

        public string GridRow2 => OsLocalization.ConvertToLocString(
            "Eng:Profit\\Loss %_" +
            "Ru:Чистый П\\У %_");

        public string GridRow3 => OsLocalization.ConvertToLocString(
            "Eng:Position count_" +
            "Ru:Количество сделок_");

        public string GridRow4 => OsLocalization.ConvertToLocString(
            "Eng:Profit Factor_" +
            "Ru:Profit Factor_");

        public string GridRow5 => OsLocalization.ConvertToLocString(
            "Eng:Recovery_" +
            "Ru:Recovery_");

        public string GridRow6 => OsLocalization.ConvertToLocString(
            "Eng:Average P\\L 1 contract_" +
            "Ru:Сред. П\\У 1 контракт_");

        public string GridRow7 => OsLocalization.ConvertToLocString(
            "Eng:Average P\\L 1 contract %_" +
            "Ru:Сред. П\\У 1 контракт %_");

        public string GridRow8 => OsLocalization.ConvertToLocString(
            "Eng:Average P\\L classic_" +
            "Ru:Сред. П\\У стандарт_");

        public string GridRow9 => OsLocalization.ConvertToLocString(
            "Eng:Average P\\L portfolio %_" +
            "Ru:Сред. П\\У капитал %_");

        public string GridRow10 => OsLocalization.ConvertToLocString(
            "Eng:Profit positions_" +
            "Ru:Прибыльных сделок_");

        public string GridRow11 => OsLocalization.ConvertToLocString(
            "Eng:Profitable %_" +
            "Ru:Прибыльных %_");

        public string GridRow12 => OsLocalization.ConvertToLocString(
            "Eng:Maximum in a row_" +
            "Ru:Максимум подряд_");

        public string GridRow13 => OsLocalization.ConvertToLocString(
            "Eng:Loss positions_" +
            "Ru:Убыточных сделок_");

        public string GridRow14 => OsLocalization.ConvertToLocString(
            "Eng:Loss Positions %_" +
            "Ru:Убыточных сделок %_");

        public string GridRow15 => OsLocalization.ConvertToLocString(
            "Eng:Max drowdown % portfolio_" +
            "Ru:Макс просадка % капитал_");

        public string GridRow16 => OsLocalization.ConvertToLocString(
            "Eng:Commission amount_" +
            "Ru:Объём комиссии_");

        public string GridRow17 => OsLocalization.ConvertToLocString(
            "Eng:Average trade time_" +
            "Ru:Среднее время удержания_");

        public string GridRow18 => OsLocalization.ConvertToLocString(
            "Eng:Sharp ratio_" +
            "Ru:Sharp ratio_");

        public string Message1 => OsLocalization.ConvertToLocString(
            "Eng:You must select a file to save!_" +
            "Ru:Необходимо выбрать файл для сохранения!_");

        public string Message2 => OsLocalization.ConvertToLocString(
            "Eng:You must select a file to load!_" +
            "Ru:Необходимо выбрать файл для загрузки!_");

        public string Message3 => OsLocalization.ConvertToLocString(
             "Eng:Are you sure you want to delete the position?_" +
             "Ru:Вы уверены что хотите удалить позицию?_");

        public string Message4 => OsLocalization.ConvertToLocString(
             "Eng:Are you sure you want to delete Positions?_" +
             "Ru:Вы уверены что хотите очистить журнал? Данные будет не восстановить!_");

        public string Message5 => OsLocalization.ConvertToLocString(
             "Eng:Are you sure you want to close all Positions at market?_" +
             "Ru:Вы уверены что хотите закрыть все позиции по маркету?_");

    }
}