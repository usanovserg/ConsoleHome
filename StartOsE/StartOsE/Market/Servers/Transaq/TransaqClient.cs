...
using StartOsE.Market.Servers.Transaq.TransaqEntity;
using RestSharp;
using System;
...
using StartOsE.Market.Servers.Transaq.TransaqEntity;
using Order = StartOsE.Market.Servers.Transaq.TransaqEntity.Order;

namespace StartOsE.Market.Servers.Transaq
{
    public class TransaqClient
    {
        ...
        public TransaqClient(string login, string password, string serverIp, string serverPort, string logPath)
        {
            ...
            _deserializer = new DotNetXmlDeserializer();
        ...
        private readonly DotNetXmlDeserializer _deserializer;
                    ...
                    if (!_newMessage.IsEmpty)
                    {
                        ...
                        if (_newMessage.TryDequeue(out data))
                        {
                            ...
                            else if (data.StartsWith("<quotes>"))
                            {
                                ...
                                var quotes = _deserializer.Deserialize<List<Quote>>(response);
                            ...
                            else if (data.StartsWith("<trades>"))
                            {
                                var response = new RestResponse();
                                response.Content = data;
                                var myTrades = _deserializer.Deserialize<List<Trade>>(response);
                            ...
                            else if (data.StartsWith("<alltrades>"))
                            {
                                var response = new RestResponse();
                                response.Content = data;
                                var allTrades = _deserializer.Deserialize<List<Trade>>(response);
                            ...
                            else if (data.StartsWith("<ticks>"))
                            {
                                var response = new RestResponse();
                                response.Content = data;
                                var newTicks = _deserializer.Deserialize<List<Tick>>(response);
                            ...
                            else if (data.StartsWith("<client"))
                            {
                                var response = new RestResponse();
                                response.Content = data;
                                var clientInfo = _deserializer.Deserialize<Client>(response);
                            ...
                            else if (data.StartsWith("<orders>"))
                            {
                                var response = new RestResponse();
                                response.Content = data;
                                var orders = _deserializer.Deserialize<List<Order>>(response);
