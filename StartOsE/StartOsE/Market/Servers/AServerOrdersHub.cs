...
namespace StartOsE.Market.Servers
{
    public class AServerOrdersHub
    {
        ...
        public void SetOrderFromStartOsE(Order order)
        {
            ...
            _ordersFromStartOsEQueue.Enqueue(order);

            if (_fullLogIsOn)
            {
                SendLogMessage("New order in StartOsE. NumUser: " + order.NumberUser
                     + " State: " + order.State
        ...
        ConcurrentQueue<Order> _ordersFromStartOsEQueue = new ConcurrentQueue<Order>();
        ...
        private void ManageOrders()
        {
            ...
            if (_orderFromApiQueue.IsEmpty == false
                || _ordersFromStartOsEQueue.IsEmpty == false)
            {
                GetOrdersFromQueue();
            }
        ...
        private void GetOrdersFromQueue()
        {
            ...
            while (_ordersFromStartOsEQueue.IsEmpty == false)
            {
                Order newOpenOrder = null;

                if(_ordersFromStartOsEQueue.TryDequeue(out newOpenOrder))
                {
            ...
            while (_orderFromApiQueue.IsEmpty == false)
            {
                ...
                {
                   // 2 перегружаем ордера которые пришли из АПИ в хранилище ордеров которые сгенерировал StartOsE
        ...
        private void TrySetOrderInHub(Order orderFromApi)
        {
            ...
            for (int i = 0;i < _ordersActiv.Count;i++)
            {
                Order curOrderFromStartOsE = _ordersActiv[i].Order;

                if(orderFromApi.NumberUser != curOrderFromStartOsE.NumberUser)
                {
                    continue;
                }
