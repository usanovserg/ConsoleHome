...
namespace MyConsole
{
    public class Program
    {
        static void Main(string[] args)
        {
            ...
            Connector.Connect();
            Connector.AddDelegate(WriteLine);

            //// Инициализируем список уровней
            ...
            // Создаем объект Position для отслеживания позиций
            Position position = new Position();

            // Подписываемся на событие изменения позиции
            position.positionChange += OnPositionChanged;

            // Задержка для работы программы
            Console.ReadLine();
        }
        ...
        static void OnPositionChanged(decimal volume, decimal price)
        {
            Console.WriteLine($"Позиция изменилась");
            Console.WriteLine($"Position Vol:\t{volume}\tAveragePrice:\t{price}\n");
        }
    }
    #endregion
    ...
    public class Connector
    {
        ...
        private void NewTrade(Trade trade) { Trades.Add(trade); }
        public void Connect() { Console.WriteLine("Connect is ExChange"); }
        public void AddDelegate(newTradeEvent metod) { NewTradeEvent = metod; }
    }
}
